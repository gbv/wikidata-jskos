...
const { mappingTypes } = require("./utils.js")
...
class WikidataWrapper {
  ...
  // TODO: move to EntityMapper class
  ...
  entityToConcept(entity) {
    ...
    for (let property in entity.claims) {
      ...
      if (mapping[0] === false) {
        ...
        concept[field] = transform(claims[0])
      } else {
        const transform = mapping[0] || itemToSetMember
        const values = claims.map(transform)
        if (concept[field]) {
  ...
  getConcepts(query) {
    ...
    const entityToConcept = this.entityToConcept.bind(this)
