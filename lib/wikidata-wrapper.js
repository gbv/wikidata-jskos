...
class WikidataJSKOSService extends ConceptSchemeSet {
  ...
  mapMapping(mapping) {
    ...
    if (!mapping.from.memberSet || !mapping.to.memberSet) return
    if (mapping.from.memberSet.length != 1 || mapping.to.memberSet.length > 1) return
    ...
    // FIXME: RVK does not work because of escaped notations (?)
    ...
    const claim = {
      mainsnak: {
        ...
        datatype: "external-id" // FIXME: ensure all schemes use this datatype (should be the case)
      }
    }

    // TODO: support mapping types

    // TODO: support/recognize qualifiers and references!

    var regex = new RegExp("^http://www\\.wikidata\\.org/entity/statement/"+id.toLowerCase()+"-[0-9a-f-]+$")
    if (mapping.uri && regex.test(mapping.uri)) {
      claim.id = mapping.uri.substr(41)
    }
