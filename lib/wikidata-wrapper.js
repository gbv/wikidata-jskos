...
class WikidataJSKOSService extends ConceptSchemeSet {
  ...
  getMapping(_id) {
    ...
    return Promise.resolve(httpRequest(url)
      ...
      .then(mappings => mappings.find(mapping => mapping.uri.endsWith(_id)))
      .then(mapping => {
        if (!mapping) {
  ...
  async saveMapping({ _id, body, user }) {
    ...
    if (_id) {
      ...
      const existingClaim = claimsBefore.find(c => c.mainsnak.property == props[0] && c.mainsnak.datavalue.value == entity.claims[props[0]].value)
      if (existingClaim) {
        entity.claims[props[0]].id = existingClaim.id
        _id = existingClaim.id.replace("$", "-")
      }
    }
    ...
    if (!_id) {
      ...
      if (newClaim) {
        _id = newClaim.id.replace("$", "-")
      } else {
        const error = new Error("Error when saving the mapping.")
        error.status = 400
        throw error
      }
    }
